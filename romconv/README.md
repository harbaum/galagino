...
before these scripts can be run. Once these are
converted you need also also [convert the audio samples](../samples).
...
machine and speed up the loading of that machine in Galagino:
...
Donkey Kong differs slightly from Pac-Man and Galaga but it's again
code ROMs, graphics and colormaps that need to be converted.
...
sounds were generated by the 8048 CPU which is now emulated in
Galagino. But some sounds were geneated using discrete logic. These
still need to be converted as described in the [samples](../samples)
directory.
